...
class MediaOrganizer:
    def __init__(self, config: OrganizerConfig) -> None:
        self.cfg = config
        self.logger = logging.getLogger("media_organizer_class")
    ...
    def run(self) -> OrganizerStats:
        ...
        self.cfg.stats.total = len(all_files)

        for idx, file_path in enumerate(all_files, start=1):
            self.cfg.stats.scanned += 1
            suffix = file_path.suffix.lower()
            self._print_progress(idx, self.cfg.stats.total, file_path.name)
            ...
            try:
                ...
                target_dir = self._build_target_dir(file_path, dt)
        ...
        if self.cfg.show_progress and self.cfg.stats.total > 0:
            print()
        self._write_report()
        return self.cfg.stats

    def _build_target_dir(self, file_path: Path, dt: datetime) -> Path:
        year_month = self.cfg.target_dir / f"{dt.year:04d}" / f"{dt.month:02d}"
        if not self.cfg.by_type:
            return year_month
        return year_month / self._classify_type(file_path)

    @staticmethod
    def _classify_type(file_path: Path) -> str:
        suffix = file_path.suffix.lower()
        if suffix == ".aae":
            return "aae"
    ...
    def _print_progress(self, index: int, total: int, filename: str) -> None:
        if not self.cfg.show_progress or total == 0:
            return
        percent = (index / total) * 100
        print(f"\rProgress: {index}/{total} ({percent:5.1f}%) | {filename[:40]}", end="", flush=True)
    ...
    def _write_report(self) -> None:
        ...
        payload = {
            ...
            "source": str(self.cfg.source_dir),
            "target": str(self.cfg.target_dir),
            "duplicates": str(self.cfg.duplicate_dir),
            "dry_run": self.cfg.dry_run,
            "by_type": self.cfg.by_type,
            "stats": {
                "total": self.cfg.stats.total,
                "scanned": self.cfg.stats.scanned,
                "moved": self.cfg.stats.moved,
                "duplicates": self.cfg.stats.duplicates,
                "skipped": self.cfg.stats.skipped,
                "errors": self.cfg.stats.errors,
            },
        }
        self.cfg.report_path.parent.mkdir(parents=True, exist_ok=True)
        self.cfg.report_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Class-based media organizer: deduplicate and sort files by year/month."
    )
    ...
    parser.add_argument("--by-type", action="store_true", help="Organize into year/month/type subfolders")
    parser.add_argument("--no-progress", action="store_true", help="Disable progress output")
    parser.add_argument("--report", type=Path, help="Write a JSON summary report to this path")
...
def main() -> int:
    ...
    logging.info(
        "Done. total=%s scanned=%s moved=%s duplicates=%s skipped=%s errors=%s",
        stats.total,
        stats.scanned,
        stats.moved,
        stats.duplicates,
